...
- Relies on **undocumented Codex internals**: the `state_5.sqlite` threads schema, the `external_agent_session_imports.json` ledger, and the deep-link routes. Verified against Codex CLI 0.145 / VS Code extension 26.721 / plugin 1.0.6 — future Codex updates could move these. The terminal resume command printed on every transfer is the always-works fallback. **Re-verified on Codex CLI 0.149.0 (2026-08-22):** every internal above is intact on it, and the transfer flow was re-run live on Windows — thread detection, dedupe, the model and effort flags, the working-directory lookup, and an absolute binary path that actually executes. Every scenario passes on it, including a genuine first import and both deep links - the latter confirmed by eye, since no test suite can see a screen ([details](docs/TESTING.md)). **Re-run again on Codex CLI 0.152.0 (2026-09-01):** the same internals and automated scenarios still pass; the two deep links were dispatched successfully but not eye-confirmed this round (screen access declined), so that specific claim still rests on the 0.149.0 confirmation, not 0.152.0.
...
- **[@ayobamiseun](https://github.com/ayobamiseun)** — [#469](https://github.com/openai/codex-plugin-cc/pull/469) is the upstream patch worth watching for this lookup bug, covering both the path-prefix mismatch and the content-hash race. Open and still unreviewed as of 2026-08-22, with a third contributor weighing in on 2026-08-12. This kit's own earlier PR ([#551](https://github.com/openai/codex-plugin-cc/pull/551)) is closed and unmerged: it normalised the path but left the content-hash race untouched, so #469 — which covers both — is the one that matters. Until it lands, this kit sidesteps both failure modes by verifying against Codex's own state instead.
