...
- Relies on **undocumented Codex internals**: the `state_5.sqlite` threads schema, the `external_agent_session_imports.json` ledger, and the deep-link routes. Verified against Codex CLI 0.145 / VS Code extension 26.721 / plugin 1.0.6 — future Codex updates could move these. The terminal resume command printed on every transfer is the always-works fallback. **Re-verification status (2026-08-22):** npm lists Codex CLI 0.149.0 as latest, four releases ahead of the 0.145.0 verified here. A static check of the shipped 0.149.0 Windows binary found every internal above still intact — same `state_5.sqlite`, same `threads` columns, same ledger fields, same `codex://threads/` route ([details](docs/TESTING.md)) — but the end-to-end run has not been repeated on it ([#8](https://github.com/AndrewAvery7/claude-codex-bridge/issues/8)).
