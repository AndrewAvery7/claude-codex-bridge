...
opens on a generative hero clip that begins dark while the light builds, which
rendered as an all-but-empty rectangle on the repository front page.

A designed card fixes the thumbnail and reads as a title rather than as a
workaround. It composites `assets/logo-dark.png` instead of redrawing the mark,
so it cannot drift from the logo the rest of the project uses, and it takes its
palette and typefaces from `tools/make-promo.py` so the card and the film that
follows are one piece of design. Everything on it is already true elsewhere in
the repository: `/to-codex` is the whole interface, the sentence is the README's
subtitle, and the strip is the badge row.
...
the released MP4 — which is how this card was actually applied, the release
asset being the only surviving copy. It shifts the audio by exactly the amount
it pushes the picture back, so the card plays silent and the film's own opening
still lands on its first frame.

## Publishing

1. `gh release upload vX.Y.Z assets/claude-codex-bridge-promo.mp4 --clobber`.
